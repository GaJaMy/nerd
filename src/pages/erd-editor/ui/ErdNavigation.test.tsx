...
it('finds hidden tables by logical name, restores them, and independently hides column details', async () => {
  ...
  await user.click(screen.getByRole('button', { name: '테이블 검색 열기' }))
  ...
  expect(node.queryByDisplayValue('id')).not.toBeInTheDocument()
  expect(node.getByDisplayValue('식별자')).toBeInTheDocument()
  await user.click(screen.getByRole('button', { name: '물리명 표시' }))
  expect(node.getByDisplayValue('users')).toBeInTheDocument()
  expect(node.queryByDisplayValue('식별자')).not.toBeInTheDocument()
  await user.click(screen.getByRole('button', { name: '논리·물리 모두 표시' }))
  expect(node.getByDisplayValue('식별자')).toBeInTheDocument()
  expect(node.getByDisplayValue('id')).toBeInTheDocument()
})
