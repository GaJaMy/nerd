...
1. 기본 **테이블 그리기** 모드에서 어두운 캔버스의 빈 공간을 드래그해 새 테이블을 만듭니다. 드래그 중 `Escape`로 취소할 수 있습니다. 테이블의 입력창 바깥을 드래그하면 위치를 옮깁니다. **화면 이동** 모드 또는 가운데/오른쪽 버튼 드래그로 화면을 이동하고 휠이나 캔버스 컨트롤로 확대·축소합니다.
2. 캔버스에서 테이블·컬럼의 논리명과 물리명을 직접 편집합니다. **논리명은 왼쪽, 물리명은 오른쪽**의 동등한 칸에 표시됩니다. 유효한 입력은 즉시 보조 패널·검색·DDL에 반영됩니다. 테이블 코멘트도 패널에서 즉시 반영됩니다. 잘못된 물리명은 초안으로 유지하며 `Escape`로 마지막 유효값을 복원합니다.
3. 테이블 하단에 마우스를 올리면 나타나는 **+**로 컬럼을 추가합니다. 키보드 포커스로도 표시되며 터치 환경에서는 항상 표시됩니다. 컬럼 이름은 즉시 반영되고, 타입·NULL 허용 여부·코멘트 등 상세 속성은 보조 패널에서 편집한 뒤 **컬럼 적용**을 누릅니다.
4. **기본키 (PK)**에서 컬럼을 체크한 순서대로 단일키 또는 복합키를 설정합니다. **기본키 적용**을 누르면 PK 컬럼은 NOT NULL이 됩니다. 모두 해제하고 적용하면 PK를 제거합니다.
5. 오른쪽 **관계 연결**을 누르고 캔버스에서 **FK 소유 테이블 → 참조 대상 테이블** 순서로 클릭합니다. 같은 테이블을 두 번 선택하면 자기 참조입니다. 이후 패널에서 참조 대상 PK 순서에 맞춰 FK 컬럼과 관계 종류를 지정하고 **관계 추가**를 누릅니다. `Escape` 또는 **연결 취소**로 취소합니다. 관계선·목록에서 기존 관계를 수정·삭제하고, **두 테이블 다시 선택**으로 연결 대상을 바꿀 수 있습니다.
6. **테이블 검색**은 물리명과 논리명을 검색합니다. 결과를 선택하면 해당 위치로 이동하며, 숨긴 테이블도 다시 표시됩니다. 이름 표시 옵션은 **논리명 / 물리명 / 논리·물리 (모두 표시)**이며 기본값은 모두 표시입니다. 데이터 타입과 코멘트도 별도로 표시하거나 숨깁니다.
